...
\begin{itemize}
  ...
  \item If the {\tt UIDNEXT} has not changed since the last time the client has opened a mailbox, the IMAP protocol says
    ...
    \begin{itemize}
      \item If the {\tt EXISTS} remains constant as well, it is clear that no deletions have taken place. This means that
...
We have mentioned message flags when describing the mailbox synchronization.  These flags allow the system and the mail
