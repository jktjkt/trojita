...
with the Qt item views.  This part works in a close collaboration with the IMAP
server, building a representation of data stored on the remote host based on
IMAP replies received and sending command to the server in response to user
actions.  Most of the IMAP protocol logic is implemented in this layer.
...
application.  Examples of such data include message parts or some mailbox
metadata allowing for quick resynchronization after a reconnect.  While its role
and the data stored therein is not critical for the operation, having a local
cache can considerably speed up operation.  A layer serving a similar purpose is
common among other e-mail clients as well.
...
a variation of a well-known Model-View-Controller design pattern.  This
...
separate models for the list of mailboxes and for each opened mailbox.  While
the latter might make more sense at a first glance and from the perspective of a
reader who just went through the IMAP standard, subsequent analysis suggests
that one big model actually fits the situation better, unless we wanted to deal
with nasty issues that would immediately pop up as a result of sharing one
Parser instance among several Models facing several mailboxes.
...
tasks like mailbox listing or even opening of them.  This listing is, however,
fetched as a result of a call from the GUI which asks the model what should be
shown in the view showing a list of mailboxes.

After the list of mailboxes is transmitted, the model tells all the attached
views that its layout has changed and that views should update what they show to
the user.  Views react by requesting more information from the Model, like
number of unread messages or a total message count for each visible mailbox.

When a user clicks a particular mailbox, another view is pointed to a subset of
the big model, now containing a list of messages in the mailbox.  Again, the
action of clicking to the mailbox does not involve any direct action; the
mailbox synchronization is invoked after the new view asks the model about how
many messages to show, and then for more data concerning those messages that are
visible in the current viewport.

One important feature implemented in \trojita which is lacking in most e-mail
clients, especially those which started with POP3 as the data model, is delayed
on-demand fetching with an intelligent preload.  Suppose a user has a big
...
user, though -- thirty thousands messages is too much to be displayed on one
...
effectively minimizes the amount of network traffic as well as local cache
bloat and the load on the remote IMAP server.
