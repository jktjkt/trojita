...
consists of three parts: {\em Envelope}, {\em Headers} and {\em Body}. The {\em
envelope} is the only relevant part for the mail exchange among the Internet
hosts; it includes basic information like addresses of the sender and recipient.
{\em Headers} provides more detailed metadata about a message, from
human-readable sender addresses and message route traces to user-defined
headers.  Some pieces of this information are used when the original delivery
fails for some reason, like the {\tt Return-Path} header.  The {\em header} also
serves as a basis for the {\em IMAP envelope data} which is explained
in chapter \ref{imap-envelope}.  Finally, the {\em message body} is what an
...
An {\em IMAP server} is a host in the Internet which provides access to a local
mail store via the standard protocol, IMAP4rev1 in this case. The IMAP server
might be located at user's employer's data center or on the user's laptop, for
example.
...
in a trusted network or that the user has provided her credentials, typically as
a user and password combination or as an X.509 certificate.
...
persistent identifier to a one message in a particular mailbox.
...
standard mechanisms for proper flagging.  In any such cases, the IMAP guarantee
...
in the same mailbox can be satisfied in client's cache.
...
same thing as the SMTP envelope.

The {\em Internal date} holds a time stamp about when was the message delivered
...
MIME standard achieves this by defining a {\em content type} for the message
body.  This content type, usually set by a header in the RFC 2822 headers of a
...
abstract content types which contains other message parts below itself.  Using
...
message that comes in several different formats, but provide the same data.  The
MUA is expected to pick one of the underlying parts and display it, perhaps
allowing the user to override this selection as a bonus.
...
7bit data.  Nowadays, 8bit data are far more common, both for representation of
...
data into a 7bit character stream and a portable mean for expressing national
...
valid.  Clients can, for example, ask for listings of mailbox tree, can get
...
order to do anything else, like retrieving messages or marking them as red.
...
manipulating the message flags) requires read-write access.

\subsection{Mailbox Synchronization}

For efficiency reasons, each client usually keeps some data in its persistent
cache.  Good candidates are message texts, cached copy of body structures or
perhaps even flags from a previous session.  Because some server replies, namely
...
compliant client has also to keep its UID-sequence number mapping up-to-speed
with the server.  Doing so for just a visible (on the GUI) part of the mailbox
