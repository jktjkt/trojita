...
separate models for list of mailboxes and for each opened mailbox.  While
the latter might make more sense at a first glance and from the perspective of a
reader who just went through the IMAP protocol standard, subsequent analysis suggests
...
tasks like mailbox listing or even opening some of them.  This listing is, however,
fetched as a result of a call from the GUI which asks the model what should be
shown in the view representing a list of mailboxes.

After the list of mailboxes is received and processed, the model tells all the attached
...
When a user clicks on a particular mailbox, another view is pointed to a subset of
the big model, now containing a list of messages in the mailbox.  Again, the
action of clicking to the mailbox does not involve any direct action in the model itself; the
mailbox synchronization is initiated only after the new view asks the model about how
...
bloat and the load on the remote IMAP server, while user barely notices any
slowdown.
