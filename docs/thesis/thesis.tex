...
interacts with; it provides views showing all mailboxes on a server, list of
messages in currently selected mailbox and the contents of e-mail messages
themselves.  The GUI is based on the Qt framework, making use of various
standard components, especially the Interview framework~\cite{qt-interview} and
the WebKit~\cite{qt-webkit} as well as certain custom widgets.
...
application.  Examples of such data include message parts or a mailbox state
...
Beneath the Model is the IMAP {\em Parser}, a component converting stream of
...
for analysing the byte stream into tokens of data like strings or numbers.

Finally, all network IO is managed by a {\em Stream} and associated classes
...
a variant of the well-known Model-View-Controller design pattern.  This
architecture provides natural separation between how data are stored and how
they are displayed.  In case of \trojita, we went a bit ahead when compared to
...
structure and actual message body parts as its nodes. Several {\em
filtering models} provide more user-oriented view, like ``all mailboxes'' or
``messages in a mailbox''.  An alternative to this one-big-tree approach were
separate models for list of mailboxes and for each opened mailbox.  While the
latter might make more sense at a first glance and from the perspective of a
reader who just went through the IMAP protocol standard, subsequent analysis
suggests that one big model actually fits the situation better, unless we wanted
to deal with nasty issues that would immediately pop up as a result of sharing
one Parser instance among several Models facing several mailboxes.  The upper
layers are not affected by this decision, as they use a strictly defined
model-view interface for actual data retrieval.

The model-view architecture subsequently affected how \trojita and her Model
operate.  Upon successful startup, for example, \trojita will not perform much
commands besides making sure a connection to the remote IMAP server enters the
authenticated state.  This might involve issuing a {\tt STARTTLS} for
establishing a secure channel, optional user authentication etc., but none of
tasks like mailbox listing or even opening some of them.  This listing is,
however, fetched as a result of a call from the GUI which asks the model what
should be shown in the view representing a list of mailboxes.

After the list of mailboxes is received and processed, the model tells all the
attached views that its layout has changed and that views should update what
they show to the user.  Views react by requesting more information from the
Model, like number of unread messages or a total message count for each visible
mailbox.
...
clients (especially those which started with POP3 as the data model) is delayed
on-demand fetching with an intelligent preloading.  Suppose a user has a big
mailbox with more than 30,000 e-mail messages.  A naive client would open a
connection to the server, select the mailbox and then fetch envelopes for all
messages in the mailbox.  Most of these messages will not be ever shown to the
...
effectively minimizes the amount of network traffic as well as local cache bloat
and the load on the remote IMAP server, while user barely notices any slowdown.
...
Parser listens to events from the underlying Socket, sending queued commands to
the server and processing incoming data as they arrive.  During the development
of \trojita (and thanks to being the first component to be actually
...
network IO, but after encountering some issues with Qt's handling of
...
conservative handling of the network IO, which closely mimics common Qt idioms.
...
ownership, the Parser makes heavy use of C++ smart pointers, as introduced in the
TR1~\cite{std-tr1}.  An alternative would be to use smart pointer implementation
from the Qt framework, the QSharedPointer.  It provides a similar interface, so
the possibility of eliminating a dependency on TR1 implementation
...
with exceptions.  Being a class derived from QObject, the Parser uses signals
and slots for communication with the rest of the application -- incoming data
are read and parsed in code connected to the {\tt readyRead()} signal from the
...
introduced only in Qt 4.5 while \trojita still supports building against the
4.4 release, albeit with reduced feature set.  Therefore, it was deemed that a
...
process a whole response at once.  In addition to more obvious and cleaner
code, this eliminated the need for blocking IO and subsequently any advantages
of a threaded design of the Parser.  Therefore, multithreading was abandoned and
new single-threaded network handler implemented.
...
Being an implementation of QAbstractItemModel, Model has to provide an interface
...
\begin{description}
    ...
    \item[TreeItemMailbox] A class representing a remote mailbox on the IMAP
        ...
        messages}.
    \item[TreeItemMsgList] A container holding all messages in mailbox
...
state} indicating that no attempt to load the data from the server was
performed, and thus the {\tt data()} implementation does not havy any real
answer to the original request.  Therefore, it submits a request to the Model to
fetch the IMAP envelope of the particular message.  However, the reply to this
...
corresponding reply.  It therefore returns an empty placeholder (or some
...
envelope.  It decides that the requested data belong to, say, message No. 666 in
the mailbox, the total message count is 670 and that no messages between 660
and 670 got fetched yet.  To enhance the user experience and minimize network
round-trips, the Model submits a request to fetch message envelopes of all these
messages, as chances are high that most of them will be queried for data soon
anyway.  As a part of this request, they are all flagged as {\em loading}.
...
TreeItemMessage instance.  The instance notices that enough data for updating
...
consumers (views).  The view, upon receiving a {\tt dataChanged()} signal from
the Model, asks for the displayable data again.  Model passes the function call
to the TreeItemMessage instance which discovers that it has the data already
available in memory and returns the real textual field (like a message Subject)
this time.
