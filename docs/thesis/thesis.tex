...
chapter~\ref{other-imap-implementations}.  Finally, the User's Guide is
available in chapter~\ref{users-guide} and the whole thesis is finished by a
wrap-up providing an overview of what we implemented and how is the result
usable.
...
Albeit the guarantee of interoperability is arguable the most important aspect
...
are widely acknowledged and its performance hard to beat.  When coupled with the
UW-imapd, an IMAP server implementation from the University of Washington from
the same author, they offer some non-standard features such as multi-mailbox
search.  The IMAP part of the code base is available as the {\tt c-client}, an
implementation of the IMAP protocol in the C programming language.

\section{KMail 3.5}

Being the default e-mail client in KDE 3.5, KMail~\cite{kmail} has a broad user
community.  However, its IMAP support might sometimes seem a bit rusty -- there
is, for one, no support for IDLE, and quite a number of bugs against the IMAP
implementation is reported at its Bugzilla.

\section{Mozilla Thunderbird 2.0}

Mozilla Thunderbird~\cite{thunderbird} has been the e-mail client of choice of
...
the mailbox performed by another session leaves much to be desired.
