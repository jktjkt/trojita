...
Some commands also result in sending another replies, the {\em untagged
replies}.  The important thing in IMAP is that such untagged replies could occur
at almost any time during the conversation and {\em not} only as a result of a
particular command.  All clients must be prepared for this situation and handle
it correctly, for there is no difference between those unilaterally generated
...
replies as a part of selecting of a mailbox.  Interesting fields for mailbox
...
If the UIDNEXT value didn't change, either, no messages were delivered to the
...
UIDs of these new messages (and FLAGS for all message in the mailbox).  If the
differences in UIDNEXT and EXISTS increments differ, a fallback to generic
synchronizing approach where client retrieves UIDs of all messages in mailbox is
...
Currently selected mailbox is typically of some interest to the user.  The
...
deliver of new, but due to lacking support in clients at that time, this feature
...
Once selected, messages in the mailbox can be queried and downloaded ot the
...
time, at least in a collapsed view with amounts of new or unread messages, and
frequent switching among several mailboxes is not a cheap operation, IMAP also
contains command for determining state of other than currently selected mailbox,
the {\em STATUS} command.  Using this command for querying state of currently
selected mailboxes is explicitly forbidden by the RFC, for a conforming client
...
Most e-mail clients therefore don't have to have all message bodies and headers
...
Following revision also specified certain extensions related to
...
7bit), IMAP doesn't use any standard encoding for mailbox names.  Instead, a
modified version of UTF-7 called simply {\em modified UTF-7} is mandated.  The
standard doesn't specify what to do if the other side uses 8bit data in mailbox
...
mail store.  This inconsistence might lead to user confusion, but can't be
...
difference leads to troubles when user wants to use POP3 in situations it wasn't
...
to work around these POP3 design limitations, they all remain just hot fixes
...
Google or regional Internet portals grew popularity.  Despite inherent
...
and accessed via the webmail interface.  Although hardly any of these services
offers any guarantee about availability or performing backups, regular users
typically aren't concerned about these issues.  Reliability of these services
is, on the other hand, typically greater than that of a typical home PC with
aging hardware, and even friendliness of the UI improved with advent of recent
web technologies.
...
all, it's a perfectly usable protocol.
