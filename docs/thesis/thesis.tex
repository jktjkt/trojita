...
create()} function, solving the problem of the nested body parts in an ellegant
way.
...
In consistence with the usual {\em expensive network} conventions, \trojita will
not load all children of the multipart/alternative body in an expensive mode.
If this mode is active, only the preferred body part is shown directly, while
others are downloaded only upon explicit user action.
...
does not really allow similar modes of operation, but is rather designed for
client-initiated actions.}, and quite a number of bugs against the IMAP
...
individual flags and not blindly specifying the resulting state of {\em all}
flags for a particular message, the operations are apparently too wide, they
target most of the messages in the mailbox, even when some of them were not
...
messages are, however, fetched as one entity at once, along with their headers.
...
concurrent connections will not be considered abuse by the server
administrators\footnote{Client developers might argue that leaving tens of
concurrent connections to the server open at the same time, each in the IDLE
command in a particular mailbox, is actually {\em better} for the server than
multiplexing the access using just one or a few of IMAP threads, but the server
administrators usually cannot be expected to have such a deep understanding of
the IMAP protocol.  Indeed, most of them would prefer to ban such a user.}.
...
\addcontentsline{toc}{chapter}{Bibliography}
    ...
    \bibitem{alpine}Alpine Messaging System, \\{\tt http://www.washington.edu/alpine/}
    ...
    \bibitem{mutt}The Mutt E-mail Client, \\
        {\tt http://www.mutt.org/}
    \bibitem{kmail}KMail: Kontact Mail, \\{\tt http://kontact.kde.org/kmail/}
