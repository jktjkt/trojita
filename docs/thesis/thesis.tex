...
The ultimate goal of this lazy population is reducing network usage and server
...
partial listing of the mailbox tree at a given hierarchy level is provided.
\trojita therefore has to fetch all the names of top-level mailboxes at once.
Requesting a listing of their children is delayed until the user clicks the
``expand'' widget next to the mailbox name, however.  Also the numbers
...
unless needed, yet an empty TreeItemMessage instance is still created for each
...
show that opening a mailbox with more than 43~thousands messages increases the
...
listing messages in the opened mailbox.

\section{The Message View}
