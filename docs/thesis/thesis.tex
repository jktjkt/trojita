...
messages in the currently selected mailbox and the contents of the e-mail messages
...
a variant of the well-known Model-View-Controller design pattern \cite{FIXME-GoF}.  This
architecture provides natural separation between how data are stored and how
they are displayed.  In case of \trojita, we went a bit ahead compared to
the common approach\footnote{FIXME: clarify what a common approach is?} and made essentially everything just a node, a part in one
...
IMAP commands besides making sure that a connection to the remote IMAP server enters the
authenticated state.  This might involve issuing a {\tt STARTTLS} command for
...
only fetched as a result of a call from the GUI which asks the model what
...
Model, e.g. a number of unread messages or a total message count for each visible
mailbox.
...
clients (especially those whose data model was designed with only POP3 in mind) is delayed
...
messages in the mailbox.  Most of these messages would never be shown to the
...
effectively minimizes the amount of network traffic as well as the local cache size
and the load on the remote IMAP server, while the user barely notices any slowdown.
...
The Parser listens to events from the underlying Socket, sending queued commands to
...
code, this eliminated the need for blocking I/O and subsequently any advantages
